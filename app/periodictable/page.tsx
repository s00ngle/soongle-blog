...
const PeriodicTablePage = () => {
  ...
  useEffect(() => {
    ...
    // 주기율표 데이터 (각 원소는 [심볼, 이름, 원자량, 열, 행] 순)
    ...
    // 각 원소 엘리먼트를 생성 후 CSS3DObject로 씬에 추가
    for (let i = 0; i < table.length; i += 5) {
      const element = document.createElement("div");
      // Tailwind의 유사 스타일 (임의 값 사용)
      element.className =
        "element w-[120px] h-[160px] font-sans text-center cursor-default";
      ...
      number.className = "number absolute top-5 right-5 text-xs";
      ...
      symbol.className = "symbol absolute left-0 right-0";
      ...
      details.className = "details absolute left-0 right-0 text-xs";
    ...
    // 버튼 클릭 시 레이아웃 변환 함수
    const transform = (targetArray: THREE.Object3D[], duration: number) => {
      TWEEN.removeAll();
  ...
  return (
    <PageContainer>
      <div className="relative bg-black">
        ...
        <div
          id="menu"
          className="absolute bottom-5 w-full text-center space-x-2"
        >
          <button
            id="table"
            className="text-[rgba(127,255,255,0.75)] bg-transparent outline outline-[rgba(127,255,255,0.75)] border-0 px-2 py-1 cursor-pointer hover:bg-[rgba(0,255,255,0.5)] active:text-black active:bg-[rgba(0,255,255,0.75)]"
          >
            TABLE
          </button>
          <button
